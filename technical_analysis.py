...
# 2. Calculate indicators (RSI, MACD, Stochastic, SMAs)
...
# 5. Plot stock price and SMA + Stochastic Oscillator
def plot_stock(df, symbol):
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10), sharex=True, gridspec_kw={'height_ratios': [3, 1]})

    # Price chart with SMA
    ax1.plot(df.index, df['Close'], label='Close Price', color='blue')
    ax1.plot(df.index, df['SMA20'], label='SMA 20', color='gray', linestyle='--')
    ax1.plot(df.index, df['SMA50'], label='SMA 50', color='orange')
    ax1.plot(df.index, df['SMA100'], label='SMA 100', color='purple', linestyle='-.')
    ax1.plot(df.index, df['SMA200'], label='SMA 200', color='green')
    ax1.set_title(f"{symbol} Price Chart with SMA and Stochastic")
    ax1.set_ylabel("Price ($)")
    ax1.legend()
    ax1.grid(True)

    # Stochastic Oscillator
    ax2.plot(df.index, df['%K'], label='%K', color='purple')
    ax2.plot(df.index, df['%D'], label='%D', color='magenta')
...
def fetch_newsapi_news(symbol, max_articles=7):
    api_key = "a76356c1492d41b18396804ba1d906b9"  # Replace with your own key for production
    url = (
...
def main():
    ...
    if st.button("Analyze"):
        ...
        start_date = (datetime.datetime.today() - datetime.timedelta(days=365*10)).strftime('%Y-%m-%d')  # last 10 years
        ...
        - **RSI (Relative Strength Index):**  
          - Below 30 indicates the stock may be **oversold** (potential buying opportunity).  
          - Above 70 indicates the stock may be **overbought** (potential selling opportunity).

        - **PE Ratio (Price-to-Earnings):**  
          - Below 15 suggests the stock is **undervalued** (good value).  
          - Above 30 suggests the stock is **overvalued** (may be expensive).

        - **Golden Cross / Death Cross:**  
          - A **Golden Cross** (50-day SMA crosses above 200-day SMA) signals a **bullish trend** (buy).  
          - A **Death Cross** signals a **bearish trend** (sell).

        - **MACD Crossover:**  
          - A **Bullish Crossover** indicates upward momentum.  
          - A **Bearish Crossover** indicates downward momentum.

        - **News Sentiment:**  
          - Positive sentiment means favorable public/company news.  
          - Negative sentiment may indicate concerns or bad news.

        - **Overall Recommendation:**  
          Based on the combination of these signals, the app suggests whether the stock looks like a **good buy, a sell, or to hold**.
